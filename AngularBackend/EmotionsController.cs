...
namespace AngularBackend
{
	public class EmotionsController
	{
		...
		public async Task<String[]> GetEmotionsFromFile(String? photoPath)
		{
			...
			try
			{
				//var photoPath = @"C:\1605303866480.jpeg";
				...
				using (Stream s = File.OpenRead(photoPath))
				{
					...
					if (facesResults.Count == 0)
						return new String[] { "No faces detected" };

					if (facesResults.Count > 1)
						return new String[] { "More then one face detected" };


					var emotions = facesResults.FirstOrDefault()?.FaceAttributes.Emotion;

					// Aggiungo le emozioni ed il rating (da ottimizzare)
					result.Add(nameof(emotions.Anger    ), emotions?.Anger     ?? 0);
					result.Add(nameof(emotions.Contempt ), emotions?.Contempt  ?? 0);
					result.Add(nameof(emotions.Disgust  ), emotions?.Disgust   ?? 0);
					result.Add(nameof(emotions.Fear     ), emotions?.Fear      ?? 0);
					result.Add(nameof(emotions.Happiness), emotions?.Happiness ?? 0);
					//result.Add(nameof(emotions.Neutral  ), emotions?.Neutral   ?? 0); Rimosso perche da qualche test mi sembra che ci sia sempre
					result.Add(nameof(emotions.Sadness  ), emotions?.Sadness   ?? 0);
					result.Add(nameof(emotions.Surprise ), emotions?.Surprise  ?? 0);
